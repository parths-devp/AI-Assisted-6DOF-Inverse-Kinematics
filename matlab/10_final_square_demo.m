...
for i=1:N
    ...
    plot3(targetPosition(1),targetPosition(2),targetPosition(3),"ro","MarkerSize",7,"MarkerFaceColor","r");
    grid on; axis equal; view(3);
    xlabel("X (m)"); ylabel("Y (m)"); zlabel("Z (m)");
    title(sprintf("AI-Assisted IK | Point %d/%d | Error = %.4f mm",i,N,positionError(i)*1000));
    drawnow;
    pause(0.02);
end
...
for j=1:6
    fprintf("J%d: %.2f deg to %.2f deg\n",j,rad2deg(min(jointTrajectory(:,j))),rad2deg(max(jointTrajectory(:,j))));
end
save("final_ai_ik_square_demo.mat","trajectory","actualPosition","jointTrajectory","positionError");